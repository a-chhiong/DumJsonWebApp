...
class TokenManager {
    ...
    async _loadTokens(idx) {
        return await Promise.all([
            vaultManager.load(`${Identity.APP_SCHEM}${Config.ACCESS_TOKEN}[${idx}]`),
            vaultManager.load(`${Identity.APP_SCHEM}${Config.REFRESH_TOKEN}[${idx}]`)
        ]);
    }
    ...
    async _saveTokens(idx, at, rt) {
        await Promise.all([
            vaultManager.save(`${Identity.APP_SCHEM}${Config.ACCESS_TOKEN}[${idx}]`, at),
            vaultManager.save(`${Identity.APP_SCHEM}${Config.REFRESH_TOKEN}[${idx}]`, rt)
        ]);
    }
    ...
    async _clearTokens(idx) {
        await Promise.all([
            vaultManager.clear(`${Identity.APP_SCHEM}${Config.ACCESS_TOKEN}[${idx}]`),
            vaultManager.clear(`${Identity.APP_SCHEM}${Config.REFRESH_TOKEN}[${idx}]`)
        ]);
    }
