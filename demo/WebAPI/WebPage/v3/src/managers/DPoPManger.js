...
class DPoPManager {
    ...
    async _loadKeys(idx) {
        return await Promise.all([
            vaultManager.loadCryptoKey(`${Identity.APP_SCHEM}${Config.PRIVATE_KEY}[${idx}]`),
            vaultManager.loadCryptoKey(`${Identity.APP_SCHEM}${Config.PUBLIC_KEY}[${idx}]`)
        ]);
    }
    ...
    async _saveKeys(idx, priv, pub) {
        await Promise.all([
            vaultManager.saveCryptoKey(`${Identity.APP_SCHEM}${Config.PRIVATE_KEY}[${idx}]`, priv),
            vaultManager.saveCryptoKey(`${Identity.APP_SCHEM}${Config.PUBLIC_KEY}[${idx}]`, pub)
        ]);
    }
    ...
    async _clearKeys(idx) {
        await Promise.all([
            vaultManager.saveCryptoKey(`${Identity.APP_SCHEM}${Config.PRIVATE_KEY}[${idx}]`),
            vaultManager.saveCryptoKey(`${Identity.APP_SCHEM}${Config.PUBLIC_KEY}[${idx}]`)
        ]);
    }
